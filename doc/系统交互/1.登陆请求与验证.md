...
#### LoginDialog::_DoLogin

用户点击登录按钮触发：`LoginDialog::_DoLogin`方法，将http登录请求操作添加到线程池中，进行登录请求。
...
1. 点击登录按钮后调用LoginDialog::_DoLogin()，生产连接登录服务器请求msg ip信息的操作任务DoLoginServerHttpOperation() ，该任务设置任务完成的回调OnHttpCallbackOperation()后放入http任务队列中
