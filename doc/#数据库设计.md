...
| fromId   | int(11) unsigned | 语音发送者id   |            |
| toId     | int(11) unsigned | 语音接受者id   |            |
| path     | varchar(255)     | 语音存储的地址 |            |
| size     | int(11) unsigned | 文件大小       |            |
| duration | int(11) unsigned | 语音时长       |            |
| created  | int(11) unsigned | 创建时间       |            |
...
| 字段         | 类型             | 说明             | 约束       |
| ------------ | ---------------- | ---------------- | ---------- |
| id           | int(11) unsigned | 发现项id         | 主键、自增 |
| itemName     | varchar(64)      | 发现项名称       |            |
| itemUrl      | varchar(64)      | 发现项url连接    |            |
| itemPriority | int(11) unsigned | 发现项显示优先级 |            |
| status       | int(11) unsigned | 状态             |            |
| created      | int(11) unsigned | 创建时间         |            |
| updated      | int(11) unsigned | 更新时间         |            |
...
| sex                | tinyint(1) unsigned | 用户的性别，1表示男性，2表示女性，0表示未知              | NOT NULL       |
| name               | varchar(32)         | 用户名                                                   | NOT NULL       |
| domain             | varchar(32)         | 用户的拼音信息                                           | NOT NULL       |
| nick               | varchar(32)         | 用户的花名、绰号等                                       | NOT NULL       |
...
| departId           | int(11) unsigned    | 用户所属部门的ID                                         | NOT NULL       |
| status             | tinyint(2) unsigned | 用户的状态，1表示试用期，2表示正式，3表示离职，4表示实习 | 默认为0        |
| created            | int(11) unsigned    | 用户的创建时间                                           | NOT NULL       |
| updated            | int(11) unsigned    | 用户的更新时间                                           | NOT NULL       |
| push_shield_status | tinyint(1) unsigned | 用户的消息推送勿扰状态，0表示关闭勿扰，1表示开启勿扰。   | NOT NULL       |
...
| 字段       | 类型             | 说明       | 约束       |
| ---------- | ---------------- | ---------- | ---------- |
| id         | int(11) unsigned | 部门id     | 主键、自增 |
| departName | varchar(64)      | 部门名称   |            |
| priority   | int(11) unsigned | 显示优先级 |            |
| parentId   | int(11) unsigned | 上级部门id |            |
| status     | int(11) unsigned | 状态       |            |
| created    | int(11) unsigned | 创建时间   |            |
| updated    | int(11) unsigned | 更新时间   |            |
...
| ------- | ------------------- | ------------------------------------------------------------ | ---------- |
| id      | int(11)             | 关系id                                                       | 主键、自增 |
| smallId | int(11) unsigned    | 用户A的id                                                    |            |
| bigId   | int(11) unsigned    | 用户B的id                                                    |            |
| status  | tinyint(1) unsigned | 关系的状态。对于用户关系，0表示正常，1表示用户A删除。对于群组关系，0表示正常，1表示被删除。 |            |
| updated | int(11) unsigned    | 更新时间                                                     |            |
| created | int(11) unsigned    | 创建时间                                                     |            |
...
| userId  | int(11) unsigned    | ⽤户id                                                       |            |
| peerId  | int(11) unsigned    | 对⽅id                                                       |            |
| type    | tinyint(2) unsigned | 该列用于表示会话的类型，1表示用户会话，2表示群组会话。       |            |
| status  | tinyint(1) unsigned | 会话的状态，对于用户会话，0表示正常，1表示用户A删除。对于群组会话，0表示正常，1表示被删除。 |            |
| updated | int(11) unsigned    | 更新时间                                                     |            |
| created | int(11) unsigned    | 创建时间                                                     |            |
...
| relateId | int(11) unsigned    | ⽤户与⽤户关系id，在IMrelationShip中。 |            |
| fromId   | int(11) unsigned    | 发送消息⽤户id                         |            |
| toId     | int(11) unsigned    | 接收消息⽤户id                         |            |
| msgId    | int(11) unsigned    | 消息id，每个relation 唯一              |            |
| content  | varchar(4096)       | 消息内容，如果是语⾳消息则存储语⾳id   |            |
| type     | tinyint(2) unsigned | 类型，⽂本or语⾳                       |            |
...
| creator    | int(11) unsigned    | 创建者用户id               |            |
| type       | tinyint(3) unsigned | 群组类型 1-固定;2-临时群   |            |
| userCnt    | int(11) unsigned    | 成员人数                   |            |
| status     | tinyint(3) unsigned | 是否删除 0-正常，1-删除    |            |
| version    | int(11) unsigned    | 群版本号                   |            |
| lastChated | int(11) unsigned    | 用于存储最后一次聊天的时间 |            |
| updated    | int(11) unsigned    | 更新时间                   |            |
| created    | int(11) unsigned    | 创建时间                   |            |
...
| groupId | int(11) unsigned    | 群的ID                                         |            |
| userId  | int(11) unsigned    | 用户的ID                                       |            |
| msgId   | int(11) unsigned    | 存储消息的ID                                   |            |
| content | varchar(4096)       | 存储消息的内容                                 |            |
| type    | tinyint(3) unsigned | 表示群消息的类型，101表示群语音，2表示文本消息 |            |
| status  | int(11) unsigned    | 存储消息的状态                                 |            |
| updated | int(11) unsigned    | 存储更新时间                                   |            |
| created | int(11) unsigned    | 存储创建时间                                   |            |
