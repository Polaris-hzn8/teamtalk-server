...
void CDBServConn::_HandleCreateGroupRsp(CImPdu* pPdu) {
  ...
  log_info("HandleCreateGroupRsp, req_id=%u, group_name=%s, result=%u", user_id,
           group_name.c_str(), result_code);

  char* response_buf = NULL;
  if (result_code != 0) {
    response_buf = PackSendCreateGroupResult(
        HTTP_ERROR_CREATE_GROUP, HTTP_ERROR_MSG[10].c_str(), group_id);
  } else {
    response_buf = PackSendCreateGroupResult(
        HTTP_ERROR_SUCCESS, HTTP_ERROR_MSG[0].c_str(), group_id);
  }
...
void CDBServConn::_HandleChangeMemberRsp(CImPdu* pPdu) {
  ...
  if (result != 0) {
    response_buf =
        PackSendResult(HTTP_ERROR_CHANGE_MEMBER, HTTP_ERROR_MSG[11].c_str());
  } else {
    response_buf =
        PackSendResult(HTTP_ERROR_SUCCESS, HTTP_ERROR_MSG[0].c_str());
  }
  pHttpConn->Send(response_buf, (uint32_t)strlen(response_buf));
  pHttpConn->Close();
