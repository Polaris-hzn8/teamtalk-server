/*
 ...
 filename: transfer_task.h
 Update Time: Thu 15 Jun 2023 00:41:14 CST
 brief:
*/
...
struct OfflineFileHeader {
  ...
  void set_to_user_id(uint32_t id) { sprintf(to_user_id, "%u", id); }
  ...
  uint32_t get_file_size() const { return string2int(std::string(file_size)); }
...
class OnlineTransferTask : public BaseTransferTask {
 public:
  ...
  uint32_t GetSeqNum() const { return mac_seq_num_; }
...
class OfflineTransferTask : public BaseTransferTask {
 public:
  ...
  uint32_t GetNextOffset() const { return SEGMENT_SIZE * transfered_idx_; }
...
struct TransferTask {
    uint32_t    transfer_mode; // FILE_TYPE_ONLINE realtime, FILE_TYPE_OFFLINE
offline / mobile string      task_id; // uuid_unparse char[37] uint32_t
from_user_id; uint32_t    to_user_id; // if offline or mobile, null bool
ready_to_recv; // bool        ready_to_send; // uint32_t    file_size; string
current_block; time_t      create_time; bool        self_destroy; CImConn*
from_conn; CImConn*    to_conn;
...
upload_packages.begin(); for ( ; itor != upload_packages.end(); ) { if
(!itor->second) { upload_packages.erase(itor++); continue ;
            }
            if (itor->second->data) {
