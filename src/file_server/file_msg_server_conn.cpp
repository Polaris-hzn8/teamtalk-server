/*
 ...
 filename: file_msg_server_conn.cpp
 Update Time: Thu 15 Jun 2023 00:40:25 CST
 brief:
*/
...
void FileMsgServerConn::_HandleMsgFileTransferReq(CImPdu* pdu)
{
    ...
    do {
        ...
        if (task_id.empty()) {
            log_info("Create task id failed");
            break;
        }
        log_info("trams_mode=%d, task_id=%s, from_id=%d, to_id=%d, file_name=%s, file_size=%d", transfer_req.trans_mode(), task_id.c_str(), from_id, to_id, transfer_req.file_name().c_str(), transfer_req.file_size());
        ...
        log_info("Create task succeed, task id %s, task type %d, from user %d, to user %d", task_id.c_str(), transfer_req.trans_mode(), from_id, to_id);
    } while (0);
