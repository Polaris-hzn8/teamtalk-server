/*
 ...
 filename: MessageContent.cpp
 Update Time: Thu 15 Jun 2023 01:07:03 CST
 brief:
*/
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    if (IM::BaseDefine::SessionType_IsValid(nSessionType)) {
      ...
      for (auto it = lsMsg.begin(); it != lsMsg.end(); ++it) {
        ...
        //                    log("userId=%u, peerId=%u, msgId=%u", nUserId,
        //                    nPeerId, it->msg_id());
      }

      log("userId=%u, peerId=%u, msgId=%u, msgCnt=%u, count=%u", nUserId,
      ...
      log("invalid sessionType. userId=%u, peerId=%u, msgId=%u, msgCnt=%u, "
          "sessionType=%u",
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    if (IM::BaseDefine::MsgType_IsValid(nMsgType)) {
      if (nMsgLen != 0) {
          ...
          if (nFromId != nToId) {
            ...
            if (nSessionId != INVALID_VALUE && nRelateId != INVALID_VALUE) {
              ...
              if (nMsgId != INVALID_VALUE) {
                ...
                log("msgId is invalid. fromId=%u, toId=%u, nRelateId=%u, "
                    "nSessionId=%u, nMsgType=%u",
                    nFromId, nToId, nRelateId, nSessionId, nMsgType);
              }
            } else {
              log("sessionId or relateId is invalid. fromId=%u, toId=%u, "
        ...
        log("fromId=%u, toId=%u, type=%u, msgId=%u, sessionId=%u", nFromId,
        ...
        log("msgLen error. fromId=%u, toId=%u, msgType=%u", nFromId, nToId,
            nMsgType);
      }
    } else {
      log("invalid msgType.fromId=%u, toId=%u, msgType=%u", nFromId, nToId,
          nMsgType);
    }
  } else {
    log("parse pb failed");
...
void getLatestMsgId(CImPdu* pPdu, uint32_t conn_uuid) {
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    if (IM::BaseDefine::SessionType_IsValid(nType)) {
      ...
      log("userId=%u, peerId=%u, sessionType=%u, msgId=%u", nUserId, nPeerId,
          nType, nMsgId);
      msgResp.set_attach_data(msg.attach_data());
      pPduResp->SetPBMsg(&msgResp);
      pPduResp->SetSeqNum(pPdu->GetSeqNum());
      ...
      log("invalid sessionType. userId=%u, peerId=%u, sessionType=%u", nUserId,
          nPeerId, nType);
    }
  } else {
    log("parse pb failed");
