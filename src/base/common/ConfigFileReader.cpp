...
bool CConfigFileReader::_LoadFile(const char* file_path) {
  ...
  if (!fp) {
    log_error("Failed to open config file[%s]: %s (errno=%d)", file_path,
  ...
  for (;;) {
    ...
    char* line = NULL;
    if ((line = fgets(buffLine, sizeof(buffLine), fp)) == NULL) break;
...
void CConfigFileReader::_ParseLine(char* line) {
  ...
  char* pos = NULL;
  if ((pos = strchr(line, '=')) == NULL) return;

  *pos = '\0';
  char* key = _TrimSpace(line);
  char* value = _TrimSpace(pos + 1);
  if (key && value) m_config_map.insert(std::make_pair(key, value));
}
