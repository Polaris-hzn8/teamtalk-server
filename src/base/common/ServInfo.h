...
void serv_init(serv_info_t* server_list, uint32_t server_count) {
  for (uint32_t i = 0; i < server_count; i++) {
    T* pConn = new T();
    // Connect
    pConn->Connect(server_list[i].server_ip.c_str(), server_list[i].server_port,
                   i);
  ...
  for (uint32_t i = 0; i < server_count; i++) {
    ...
    if (!pConn) {
      ...
      if (server_list[i].idle_cnt >= server_list[i].reconnect_cnt) {
        pConn = new T();
        pConn->Connect(server_list[i].server_ip.c_str(),
                       server_list[i].server_port, i);
        server_list[i].serv_conn = pConn;
      }
    }
  }
...
// 重置服务器连接对象
template <class T>
void serv_reset(serv_info_t* server_list, uint32_t server_count,
                uint32_t serv_idx) {
