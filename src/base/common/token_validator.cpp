...
void md5(const uint8_t* initial_msg, size_t initial_len, uint8_t* digest) {
  ...
  // append "0" bits
  for (offset = initial_len + 1; offset < new_len; offset++) msg[offset] = 0;
...
int genToken(unsigned int uid, time_t time_offset, char* md5_str_buf) {
  ...
  snprintf(t_buf, sizeof(t_buf), "%04d-%02d-%02d-%02d", systemTime.wYear,
  ...
  snprintf(t_buf, sizeof(t_buf), "%04d-%02d-%02d-%02d", tm->tm_year + 1900,
           tm->tm_mon + 1, tm->tm_mday, tm->tm_hour);
#endif

  snprintf(tmp_buf, sizeof(tmp_buf), "%s_%u_%s_%s", AUTH_ENCRYPT_KEY, uid,
           t_buf, AUTH_ENCRYPT_KEY);
...
bool IsTokenValid(uint32_t user_id, const char* token) {
  ...
  if (!strcmp(token, token1) || !strcmp(token, token2) ||
      !strcmp(token, token3))
    return true;
  return false;
}
