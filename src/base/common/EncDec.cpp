...
int CAes::Encrypt(const char* pInData, uint32_t nInLen, char** ppOutData,
                  uint32_t& nOutLen) {
...
int CAes::Decrypt(const char* pInData, uint32_t nInLen, char** ppOutData,
                  uint32_t& nOutLen) {
  if (pInData == NULL || nInLen <= 0) return -1;
...
void CMd5::MD5_Calculate(const char* pContent, unsigned int nLen, char* md5) {
  ...
  for (int i = 0; i < 16; ++i) sprintf(md5 + (i * 2), "%02x", dist[i]);
