...
size_t write_data_binary(void* ptr, size_t size, size_t nmemb,
                         AudioMsgInfo* pAudio) {
...
// POST
CURLcode CHttpClient::Post(const std::string& strUrl,
...
std::string CHttpClient::UploadByteFile(const std::string& strUrl, void* pData,
                                        int nSize) {
  std::string strRet;
  if (strUrl.empty()) return strRet;

  CURL* curl = curl_easy_init();
  if (!curl) return strRet;
...
bool CHttpClient::DownloadByteFile(const std::string& url,
                                   AudioMsgInfo* pAudioMsg) {
  CURL* curl = curl_easy_init();
