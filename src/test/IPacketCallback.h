...
class IPacketCallback {
 public:
  ...
  virtual void onError(uint32_t nSeqNo, uint32_t nCmd,
                       const string& strMsg) = 0;
  virtual void onLogin(uint32_t nSeqNo, uint32_t nResultCode, string& strMsg,
                       IM::BaseDefine::UserInfo* pUser = NULL) = 0;
  virtual void onGetChangedUser(
      uint32_t nSeqNo, const list<IM::BaseDefine::UserInfo>& lsUser) = 0;
  virtual void onGetUserInfo(uint32_t nSeqNo,
                             const list<IM::BaseDefine::UserInfo>& lsUser) = 0;
  virtual void onSendMsg(uint32_t nSeqNo, uint32_t nUserId, uint32_t nRecvId,
                         IM::BaseDefine::SessionType nType,
                         uint32_t nMsgId) = 0;
  virtual void onGetUnreadMsgCnt(
      uint32_t nSeqNo, uint32_t nUserId, uint32_t nTotalCnt,
      const list<IM::BaseDefine::UnreadInfo>& lsUnreadCnt) = 0;
  virtual void onGetRecentSession(
      uint32_t nSeqNo, uint32_t nUserId,
      const list<IM::BaseDefine::ContactSessionInfo>& lsSession) = 0;
  virtual void onGetMsgList(uint32_t nSeqNo, uint32_t nUserId, uint32_t nPeerId,
                            ...
                            const list<IM::BaseDefine::MsgInfo>& lsMsg) = 0;
