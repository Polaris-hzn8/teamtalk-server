...
enum Format {
  ...
  JXMLex =
      3,  // XML output, JXMLex format. see https://github.com/r-lyeh/JXMLex
...
#undef $number
  ...
  void import(const Value &other) {
    if (this != &other) switch (other.type_) {
        case NULL_:
          import(Null());
          break;
        case BOOL_:
  ...
  template <typename T>
  Value(const T &t) : type_(INVALID_) {
    import(t);
  }
  template <size_t N>
  Value(const char (&t)[N]) : type_(INVALID_) {
    import(std::string(t));
  }
