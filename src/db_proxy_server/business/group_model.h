/*
 ...
 Update Time: Thu 15 Jun 2023 00:30:58 CST
 brief:
*/
...
class CGroupModel {
 ...
 public:
  uint32_t createGroup(uint32_t nUserId, const std::string& strGroupName,
                       const std::string& strGroupAvatar, uint32_t nGroupType,
  ...
  void getUserGroup(uint32_t nUserId,
                    ...
                    uint32_t nGroupType);
  void getUserGroupIds(uint32_t nUserId, std::list<uint32_t>& lsGroupId,
  ...
  bool setPush(uint32_t nUserId, uint32_t nGroupId, uint32_t nType,
               uint32_t nStatus);
  void getPush(uint32_t nGroupId, std::list<uint32_t>& lsUser,
 ...
 private:
  ...
  bool insertNewGroup(uint32_t reqUserId, const std::string& groupName,
                      const std::string& groupAvatar, uint32_t groupType,
  ...
  void getGroupVersion(std::list<uint32_t>& lsGroupId,
                       std::list<IM::BaseDefine::GroupVersionInfo>& lsGroup,
                       uint32_t nGroupType);
  bool hasModifyPermission(uint32_t nUserId, uint32_t nGroupId,
                           IM::BaseDefine::GroupModifyType nType);
  bool addMember(uint32_t nGroupId, std::set<uint32_t>& setUser,
                 std::list<uint32_t>& lsCurUserId);
  bool removeMember(uint32_t nGroupId, std::set<uint32_t>& setUser,
                    std::list<uint32_t>& lsCurUserId);
