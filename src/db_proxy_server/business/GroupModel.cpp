/*
 ...
 filename: GroupModel.cpp
 Update Time: Thu 15 Jun 2023 00:30:47 CST
 brief:
*/
...
CGroupModel* CGroupModel::getInstance() {
  if (!m_pInstance) m_pInstance = new CGroupModel();
  return m_pInstance;
}
...
uint32_t CGroupModel::createGroup(uint32_t nUserId, const string& strGroupName,
  ...
  do {
    ...
    if (!bRet) {
      log_info("reset msgId failed. groupId=%u", nGroupId);
    }
        ...
        while (pResultSet->Next()) {
          ...
          if (mapGroupId[nGroupId].version() < nVersion) {
            ...
            if (IM::BaseDefine::GroupType_IsValid(nGroupType)) {
              ...
              log_info("invalid groupType. groupId=%u, groupType=%u", nGroupId,
                       nGroupType);
            }
          }
        }
  ...
  if (hasModifyPermission(nUserId, nGroupId, nType)) {
    switch (nType) {
      ...
      case IM::BaseDefine::GROUP_MODIFY_TYPE_DEL:
        ...
        break;
      default:
        log_info("unknown type:%u while modify group.%u->%u", nType, nUserId,
                 nGroupId);
        break;
    }
    ...
    log_info("user:%u has no permission to modify group:%u", nUserId, nGroupId);
  }
  return bRet;
}
...
bool CGroupModel::isInGroup(uint32_t nUserId, uint32_t nGroupId) {
  ...
  if (pCacheConn) {
    ...
    log_info("no cache connection for group_member");
  }
  return bRet;
}
...
bool CGroupModel::setPush(uint32_t nUserId, uint32_t nGroupId, uint32_t nType,
  ...
  if (!isInGroup(nUserId, nGroupId)) {
    log_info("user:%d is not in group:%d", nUserId, nGroupId);
    return bRet;
    ;
  }
  ...
  if (pCacheConn) {
    ...
    log_info("no cache connection for group_set");
  }
  return bRet;
}
...
//     if(pDBConn)
//     {
//         string strSql = "select id from IMGroup where id=" +
//         int2string(nGroupId)+" and status=0"; CResultSet* pResultSet =
//         pDBConn->ExecuteQuery(strSql.c_str()); if(pResultSet &&
//         pResultSet->Next())
//         {
//             bRet =  true;
//             delete pResultSet;
