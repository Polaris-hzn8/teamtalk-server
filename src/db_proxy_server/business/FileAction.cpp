/*
 ...
 filename: FileAction.cpp
 Update Time: Thu 15 Jun 2023 00:29:47 CST
 brief:
*/
...
void hasOfflineFile(CImPdu* pPdu, uint32_t conn_uuid) {
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    for (std::list<IM::BaseDefine::OfflineFileInfo>::iterator it =
             lsOffline.begin();
         it != lsOffline.end(); ++it) {
...
void addOfflineFile(CImPdu* pPdu, uint32_t conn_uuid) {
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    log_info("fromId=%u, toId=%u, taskId=%s, fileName=%s, fileSize=%u", nUserId,
...
void delOfflineFile(CImPdu* pPdu, uint32_t conn_uuid) {
  ...
  if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    ...
    log_info("fromId=%u, toId=%u, taskId=%s", nUserId, nToId,
             strTaskId.c_str());
  }
}
