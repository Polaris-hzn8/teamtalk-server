...
int main(int argc, char* argv[]) {
  ...
  char* str_register_center_port =
      config_file.GetConfigName("RegisterCenterPort");
  ...
  if (!str_register_center_ip || !str_register_center_port || !str_service_id ||
      !str_service_dir || !str_host_ip || !str_register_ttl) {
  ...
  // reg_info.reg_center_addr = str_register_center_ip + ":" +
  // str_register_center_port;  / 注册中心地址
  reg_info.service_id =
      str_service_id;  // 该服务标识，前4字符标识同类型服务，后4字符标识服务序号
