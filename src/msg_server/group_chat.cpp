...
void CGroupChat::HandleClientGroupNormalRequest(CImPdu* pPdu,
                                                CMsgConn* pFromConn) {
  ...
  if (pDBConn) {
    ...
    msg.set_attach_data((uchar_t*)attach_data.GetBuffer(),
...
void CGroupChat::HandleGroupNormalResponse(CImPdu* pPdu) {
  ...
  log_info("HandleGroupNormalResponse, user_id=%u, group_cnt=%u. ", user_id,
           group_cnt);

  msg.clear_attach_data();
  pPdu->SetPBMsg(&msg);
  CMsgConn* pConn = CImUserManager::GetInstance()->GetMsgConnByHandle(
      user_id, attach_data.GetHandle());
  if (pConn) {
    pConn->SendPdu(pPdu);
  }
}

void CGroupChat::HandleClientGroupInfoRequest(CImPdu* pPdu,
                                              CMsgConn* pFromConn) {
  ...
  uint32_t user_id = pFromConn->GetUserId();
  uint32_t group_cnt = msg.group_version_list_size();
  log_info("HandleClientGroupInfoRequest, user_id=%u, group_cnt=%u. ", user_id,
           group_cnt);
  CPduAttachData attach_data(ATTACH_TYPE_HANDLE, pFromConn->GetHandle(), 0,
                             NULL);
...
void CGroupChat::HandleGroupInfoResponse(CImPdu* pPdu) {
  ...
  CPduAttachData pduAttachData((uchar_t*)msg.attach_data().c_str(),
                               msg.attach_data().length());

  log_info("HandleGroupInfoResponse, user_id=%u, group_cnt=%u. ", user_id,
           group_cnt);
  ...
  if (pduAttachData.GetPduLength() > 0 && group_cnt > 0) {
    ...
    CHECK_PB_PARSE_MSG(msg2.ParseFromArray(pduAttachData.GetPdu(),
                                           pduAttachData.GetPduLength()));
    CImPdu pdu;
    pdu.SetPBMsg(&msg2);
    ...
    for (uint32_t i = 0; i < group_info.group_member_list_size(); i++) {
      ...
      CImUser* pToImUser =
          CImUserManager::GetInstance()->GetImUserById(member_user_id);
      if (pToImUser) {
        CMsgConn* pFromConn = NULL;
        if (member_user_id == user_id) {
          uint32_t reqHandle = pduAttachData.GetHandle();
          if (reqHandle != 0)
            pFromConn = CImUserManager::GetInstance()->GetMsgConnByHandle(
                user_id, reqHandle);
        }
    ...
    // 正常获取群信息的返回
    CMsgConn* pConn = CImUserManager::GetInstance()->GetMsgConnByHandle(
        user_id, pduAttachData.GetHandle());
    if (pConn) {
      msg.clear_attach_data();
      pPdu->SetPBMsg(&msg);
      pConn->SendPdu(pPdu);
...
void CGroupChat::HandleGroupMessage(CImPdu* pPdu) {
  ...
  if (msg_id == 0) {
    log_info("HandleGroupMsg, write db failed, %u->%u. ", from_user_id,
             to_group_id);
    return;
  }
  ...
  log_info("HandleGroupMsg, %u->%u, msg id=%u. ", from_user_id, to_group_id,
           msg_id);
  ...
  CPduAttachData pduAttachData(ATTACH_TYPE_HANDLE_AND_PDU,
                               attach_data.GetHandle(), pPdu->GetBodyLength(),
                               pPdu->GetBodyData());

  IM::Group::IMGroupInfoListReq msg3;
  msg3.set_user_id(from_user_id);
  IM::BaseDefine::GroupVersionInfo* group_version_info =
      msg3.add_group_version_list();
...
void CGroupChat::HandleGroupMessageBroadcast(CImPdu* pPdu) {
  ...
  string msg_data = msg.msg_data();
  uint32_t msg_id = msg.msg_id();
  log_info("HandleGroupMessageBroadcast, %u->%u, msg id=%u. ", from_user_id,
           to_group_id, msg_id);
...
void CGroupChat::HandleClientGroupCreateRequest(CImPdu* pPdu,
                                                CMsgConn* pFromConn) {
...
void CGroupChat::HandleGroupCreateResponse(CImPdu* pPdu) {
  ...
  log_info(
      ...
      user_id, result, group_id, group_name.c_str(), user_cnt);
...
void CGroupChat::HandleClientGroupChangeMemberRequest(CImPdu* pPdu,
                                                      CMsgConn* pFromConn) {
...
void CGroupChat::HandleClientGroupShieldGroupRequest(CImPdu* pPdu,
                                                     CMsgConn* pFromConn) {
...
void CGroupChat::HandleGroupShieldGroupResponse(CImPdu* pPdu) {
  ...
  uint32_t group_id = msg.group_id();
  log_info(
      "HandleGroupShieldGroupResponse, result: %u, user_id: %u, group_id: %u. ",
...
void CGroupChat::HandleGroupGetShieldByGroupResponse(CImPdu* pPdu) {
  ...
  log_info("HandleGroupGetShieldByGroupResponse, shield_status_list_cnt: %u. ",
           shield_status_list_cnt);

  IM::Server::IMGetDeviceTokenReq msg2;
  msg2.set_attach_data((uchar_t*)msg.attach_data().c_str(),
                       msg.attach_data().length());
  for (uint32_t i = 0; i < shield_status_list_cnt; i++) {
    ...
    if (shield_status.shield_status() == 0) {
      ...
      log_info("user_id: %u shield group, group id: %u. ",
