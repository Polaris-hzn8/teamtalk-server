...
void msg_conn_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    if (cur_time > g_last_stat_tick + LOG_MSG_STAT_INTERVAL) {
        ...
        log_info("up_msg_cnt=%u, up_msg_miss_cnt=%u, down_msg_cnt=%u, down_msg_miss_cnt=%u ",
...
void CMsgConn::_HandleClientRecentContactSessionRequest(CImPdu* pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log_info("HandleClientRecentContactSessionRequest, user_id=%u, latest_update_time=%u. ", GetUserId(), msg.latest_update_time());
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu) {
    ...
    if (g_log_msg_toggle) log_info("HandleClientMsgData, %d->%d, msg_type=%u, msg_id=%u. ", GetUserId(), to_session_id, msg_type, msg_id);
...
void CMsgConn::_HandleClientGetMsgListRequest(CImPdu* pPdu)
{
    ...
    log_info("HandleClientGetMsgListRequest, req_id=%u, session_type=%u, session_id=%u, msg_id_begin=%u, msg_cnt=%u. ",
...
void CMsgConn::_HandleClientGetMsgByMsgIdRequest(CImPdu* pPdu)
{
    ...
    log_info("_HandleClientGetMsgByMsgIdRequest, req_id=%u, session_type=%u, session_id=%u, msg_cnt=%u.",
...
void CMsgConn::_HandleClientMsgReadAck(CImPdu* pPdu)
{
    ...
    uint32_t session_id = msg.session_id();
    uint32_t msg_id = msg.msg_id();
    log_info("HandleClientMsgReadAck, user_id=%u, session_id=%u, msg_id=%u, session_type=%u. ", GetUserId(), session_id, msg_id, session_type);
...
void CMsgConn::_HandleClientGetLatestMsgIDReq(CImPdu* pPdu)
{
    ...
    uint32_t session_type = msg.session_type();
    uint32_t session_id = msg.session_id();
    log_info("HandleClientGetMsgListRequest, user_id=%u, session_id=%u, session_type=%u. ", GetUserId(), session_id, session_type);
...
void CMsgConn::_HandleRegistRequest(CImPdu* pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log_info("user_name=%s, password=%s, sex=%u, nick=%s, avatar=%s, phone=%s, email=%s",
