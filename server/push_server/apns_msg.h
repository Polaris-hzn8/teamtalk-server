...
/* An arbitrary, opaque value that identifies this notification. This identifier
   is used for reporting errors to your server. */
...
/* A UNIX epoch date expressed in seconds (UTC) that identifies when the
   ...
   non-zero, APNs stores the notification tries to deliver the notification at
   least once. Specify zero to indicate that the notification expires
...
/* 5 The push message is sent at a time that conserves power on the device
 * receiving it. */
