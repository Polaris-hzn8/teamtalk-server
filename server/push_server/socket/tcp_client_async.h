...
class SOCKET_IO_DECLARE_CLASS CTCPClientAsync : public CBaseIOStream {
 public:
  ...
  virtual const char* GetRemoteIP() const { return m_szRemoteIP; }
  void SetRemotePort(int32_t nPort) { m_nRemotePort = nPort; }
  virtual int32_t GetRemotePort() { return m_nRemotePort; }

  /*uint32_t nsockid*/
  sigslot::signal1<uint32_t> DoConnect;

  /*uint32_t nsockid, const char* szBuf, int32_t nBufSize, const char* szIP,
   * int32_t nPort*/
