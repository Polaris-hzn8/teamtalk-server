 ...
 *
 *	@note
 *实现一个环形内存区的类，该类线程不安全，需要外部加锁保护,但是对于分别只有一个
 *        读和写的线程是安全的
 *	@note 历史记录：
 *	@note V1.0.0  创建文件
 */
...
class CBufferLoop {
 public:
   ...
   *nbuffer_size, int* nreal_buffer_size)
  ...
  /**	@fn	bool CBufferLoop::append_buffer(const char* pbuffer, int
   *nbuffer_size)
   *	@brief
   *	@param[in] pbuffer
  ...
  /**	@fn	bool CBufferLoop::get_buffer(char* pbuffer, int nbuffer_size,
   *int* nreal_buffer_size)
