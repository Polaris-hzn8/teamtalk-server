...
int main(int argc, char* argv[]) {
  ...
  // 监听客户端连接
  for (const std::string& ip : cfg.client_listen_addresses()) {
    ret = ttnetlib::netlib_listen(ip.c_str(), cfg.client_port(), client_callback, nullptr);
    if (ret == ttnetlib::NETLIB_ERROR)
      return ret;
  }

  // 监听 msg_server
  for (const std::string& ip : cfg.msg_server_listen_addresses()) {
    ret = ttnetlib::netlib_listen(ip.c_str(), cfg.msg_server_port(), msg_serv_callback, nullptr);
    if (ret == ttnetlib::NETLIB_ERROR)
      return ret;
  }

  // 监听 HTTP连接
  for (const std::string& ip : cfg.http_listen_addresses()) {
    ret = ttnetlib::netlib_listen(ip.c_str(), cfg.http_port(), http_callback, nullptr);
    if (ret == ttnetlib::NETLIB_ERROR)
      return ret;
  }
