/*
 ...
 Update Time: Thu 15 Jun 2023 00:44:45 CST
 brief:
*/
...
void CHttpConn::OnRead() {
  ...
  if (m_cHttpParser.IsReadAll()) {
    std::string url = m_cHttpParser.GetUrl();
    if (strncmp(url.c_str(), "/msg_server", 11) == 0) {  // 路由判断
      std::string content = m_cHttpParser.GetBodyContent();
...
void CHttpConn::_HandleMsgServRequest(std::string& url, std::string& post_data) {
  ...
  if (MsgServerRegistry::Instance().Empty())  // 没有可用的msg_server
  {
    Json::Value value;
    value["code"] = 1;
    value["msg"] = "没有可用的消息服务器";
    std::string strContent = value.toStyledString();
  ...
  if (!found) {
    ...
    if (ttipparser::is_telcome(GetPeerIP())) {
      ...
      value["priorIP"] = std::string(target_msg_serv.ip_addr2);
      value["backupIP"] = std::string(target_msg_serv.ip_addr1);
      value["msfsPrior"] = cfg.msfs_url();
      value["msfsBackup"] = cfg.msfs_url();
    }
    value["discovery"] = cfg.discovery();
    value["port"] = ttstr::int2string(target_msg_serv.port);
    std::string strContent = value.toStyledString();
    char* szContent = new char[HTTP_RESPONSE_HTML_MAX];
