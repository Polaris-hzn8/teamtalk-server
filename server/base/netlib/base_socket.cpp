...
void CBaseSocket::SetSendBufSize(uint32_t send_size) {
  // 设置发送缓冲区大小
  int ret =
      setsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, SOCKOPT_CAST(&send_size), 4);
  if (ret == SOCKET_ERROR)
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size) {
  // 设置接收缓冲区大小
  int ret =
      setsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, SOCKOPT_CAST(&recv_size), 4);
  if (ret == SOCKET_ERROR)
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port,
  ...
  if (m_socket == INVALID_SOCKET) {
    log_error("socket failed, err_code=%d, server_ip=%s, port=%u",
  ...
  if (ret == SOCKET_ERROR) {
    log_error("bind failed, err_code=%d, server_ip=%s, port=%u",
              _GetErrorCode(), server_ip, port);
  ...
  if (ret == SOCKET_ERROR) {
    log_error("listen failed, err_code=%d, server_ip=%s, port=%u",
              _GetErrorCode(), server_ip, port);
    closesocket(m_socket);
    return NETLIB_ERROR;
  }
  ...
  CEventDispatch::Instance()->AddEvent(
      m_socket, SOCKET_READ | SOCKET_EXCEP);  // 监听套接字读事件
...
net_handle_t CBaseSocket::Connect(const char* server_ip, uint16_t port,
  ...
  if ((ret == SOCKET_ERROR) && (!_IsBlock(_GetErrorCode()))) {
    log_error("connect failed, err_code=%d, server_ip=%s, port=%u",
              _GetErrorCode(), server_ip, port);
    closesocket(m_socket);
    return NETLIB_INVALID_HANDLE;
  }
...
void CBaseSocket::OnRead() {
  if (m_state == SOCKET_STATE_LISTENING) {
    ...
    if ((SOCKET_ERROR == ret) || (avail == 0)) {
      /* 发生错误或没有可读数据 */
      m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t)m_socket,
                 NULL);
    } else {
      /* 正常有可读数据 */
      m_callback(m_callback_data, NETLIB_MSG_READ, (net_handle_t)m_socket,
                 NULL);
    }
  }
}
...
void CBaseSocket::OnWrite() {
  ...
  if (m_state == SOCKET_STATE_CONNECTING) {
    ...
    if (error) {
      ...
      m_callback(m_callback_data, NETLIB_MSG_CONFIRM, (net_handle_t)m_socket,
                 NULL);
    }
  } else {
    /* 套接字不处于连接中状态 */
