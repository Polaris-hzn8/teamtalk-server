/*
 ...
 Update Time: Thu 15 Jun 2023 00:54:36 CST
 brief:
*/
...
void CFileHandler::HandleClientFileHasOfflineReq(ttconnection::CMsgConn* pMsgConn, ttnetlib::CImPdu* pPdu) {
...
void CFileHandler::HandleClientFileAddOfflineReq(ttconnection::CMsgConn* pMsgConn, ttnetlib::CImPdu* pPdu) {
  ...
  if (pFileConn) {
    ...
    msg2.set_trans_mode(ttidlbase::FILE_TYPE_OFFLINE);
    msg2.set_offline_ready(1);
    for (list<ttidlbase::IpAddr>::const_iterator it = file_addr_list->begin(); it != file_addr_list->end(); it++) {
    ...
    ttconnection::CRouteServConn* pConn = ttconnection::get_route_serv_conn();
    if (pConn) {
      pConn->SendPdu(&pdu);
    }
  }
...
void CFileHandler::HandleFileHasOfflineRes(ttnetlib::CImPdu* pPdu) {
  ...
  if (pFileConn) {
    ...
    for (list<ttidlbase::IpAddr>::const_iterator it = ip_list->begin(); it != ip_list->end(); it++) {
      ttidlbase::IpAddr ip_addr_tmp = *it;
      ttidlbase::IpAddr* ip_addr = msg.add_ip_addr_list();
      ip_addr->set_ip(ip_addr_tmp.ip());
      ip_addr->set_port(ip_addr_tmp.port());
    }
  } else {
...
void CFileHandler::HandleFileNotify(ttnetlib::CImPdu* pPdu) {
  ...
  log_info(
    ...
    trans_mode,
    offline_ready);
  ttuser::CImUser* pUser = ttuser::CImUserManager::GetInstance()->GetImUserById(to_user_id);
  if (pUser) {
