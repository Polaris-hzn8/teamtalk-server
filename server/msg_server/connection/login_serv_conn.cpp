/*
 ...
 filename: LoginServConn.cpp
 Update Time: Thu 15 Jun 2023 00:56:32 CST
 brief:
*/
...
void CLoginServConn::Connect(const char* server_ip, uint16_t server_port, uint32_t serv_idx) {
  ...
  m_serv_idx = serv_idx;
  m_handle =
    ttnetlib::netlib_connect(server_ip, server_port, ttnetlib::imconn_callback, (void*)&g_login_server_conn_map);
...
void CLoginServConn::OnClose() {
  log_info("login server conn onclose, from handle=%d ", m_handle);
  Close();
}
...
void CLoginServConn::OnTimer(uint64_t curr_tick) {
  ...
  if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
    log_info("conn to login server timeout ");
    Close();
  }
}
