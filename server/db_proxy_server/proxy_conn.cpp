/*
 ...
 filename: ProxyConn.cpp
 Update Time: Wed 14 Jun 2023 23:05:53 CST
 brief:
*/
...
CProxyConn::CProxyConn() {
  ...
  if (m_uuid == 0) m_uuid = ++CProxyConn::s_uuid_alloctor;
...
void CProxyConn::OnConnect(net_handle_t handle) {
  ...
  log_info("connect from %s:%d, handle=%d", m_peer_ip.c_str(), m_peer_port,
           m_handle);
}
...
void CProxyConn::OnRead() {
  for (;;) {
    ...
    int ret =
        netlib_recv(m_handle, m_in_buf.GetBuffer() + m_in_buf.GetWriteOffset(),
                    READ_BUF_SIZE);
    if (ret <= 0) break;
  ...
  try {
    ...
    log_info(
        "!!!catch exception, err_code=%u, err_msg=%s, close the connection ",
        ex.GetErrorCode(), ex.GetErrorMsg());
    OnClose();
  }
}

void CProxyConn::OnClose() { Close(); }
...
void CProxyConn::HandlePduBuf(uchar_t* pdu_buf, uint32_t pdu_len) {
  ...
  if (pPdu->GetCommandId() == IM::BaseDefine::CID_OTHER_HEARTBEAT) return;
  ...
  while (!s_response_pdu_list.empty()) {
    ...
    if (pConn) {
      if (pResp->pPdu) {
        ...
        log_info("close connection uuid=%d by parse pdu error\b",
                 pResp->conn_uuid);
        pConn->Close();
      }
    }
...
///////////////////////////////////////////////////////////
void proxy_timer_callback(void* callback_data, uint8_t msg, uint32_t handle,
                          void* pParam) {
  uint64_t cur_time = get_tick_count();
  for (ConnMap_t::iterator it = g_proxy_conn_map.begin();
       it != g_proxy_conn_map.end();) {
...
void proxy_loop_callback(void* callback_data, uint8_t msg, uint32_t handle,
                         void* pParam) {
 ...
 */
void exit_callback(void* callback_data, uint8_t msg, uint32_t handle,
                   void* pParam) {
...
static void sig_handler(int sig_no) {
  if (sig_no == SIGTERM) {
    ...
    for (ConnMap_t::iterator it = g_proxy_conn_map.begin();
         it != g_proxy_conn_map.end(); it++) {
      CProxyConn* pConn = (CProxyConn*)it->second;
      pConn->SendPdu(&cPdu);
    }
    // Add By ZhangYuanhao
    // Before stop we need to stop the sync thread,otherwise maybe will not sync
    // the internal data any more
...
CProxyConn* get_proxy_conn_by_uuid(uint32_t uuid) {
  ...
  if (it != g_uuid_conn_map.end()) pConn = (CProxyConn*)it->second;
  return pConn;
}
