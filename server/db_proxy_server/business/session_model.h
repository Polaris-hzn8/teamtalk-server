/*
 ...
 filename: SessionModel.h
 Update Time: Thu 15 Jun 2023 00:32:49 CST
 brief:
*/
...
class CSessionModel {
 public:
  ...
  void getRecentSession(
      uint32_t userId, uint32_t lastTime,
      std::list<IM::BaseDefine::ContactSessionInfo>& lsContact);
  uint32_t getSessionId(uint32_t nUserId, uint32_t nPeerId, uint32_t nType,
                        bool isAll);
 ...
 private:
  CSessionModel(){};
  void fillSessionMsg(uint32_t nUserId,
                      std::list<IM::BaseDefine::ContactSessionInfo>& lsContact);
