/*
 ...
 Update Time: Thu 15 Jun 2023 00:50:52 CST
 brief:
        Multimedia Small File Storage System
        File Manager Singleton implement to manage file store and download
 operation
*/
...
int FileManager::initDir() {
  ...
  for (int i = 0; i <= FIRST_DIR_MAX; i++) {
    ...
    if (code && (errno != EEXIST)) {
      log_info("Create dir[%s] error[%d]", tmp.c_str(), errno);
      return -1;
    }
    for (int j = 0; j <= SECOND_DIR_MAX; j++) {
      ...
      if (code && (errno != EEXIST)) {
        log_info("Create dir[%s] error[%d]", tmp2.c_str(), errno);
        return -1;
      }
...
int FileManager::getRelatePathByUrl(const string& url, string& path) {
  ...
  if (string::npos == pos) {
    log_info("Url [%s] format illegal.", url.c_str());
    return -1;
  }
  path = url.substr(pos);
  return 0;
...
int FileManager::getAbsPathByUrl(const string& url, string& path) {
  ...
  if (getRelatePathByUrl(url, relate)) {
    log_info("Get path from url[%s] error", url.c_str());
    return -1;
  }
...
FileManager::Entry* FileManager::getOrCreateEntry(const std::string& url,
  ...
  if (it != m_map.end()) {
    log_info("the map has the file while url:%s", url.c_str());
    m_cs.Leave();
    return it->second;
  }
  if (!create) {
  ...
  if (getAbsPathByUrl(url, path)) {
    log_info("Get abs path from url[%s] error", url.c_str());
    m_cs.Leave();
    return NULL;
  }
  ...
  if (ret) {
    log_info("read file error while url:%s", url.c_str());
    delete e;
    e = NULL;
    delete tmpFile;
    tmpFile = NULL;
  ...
  if (result.second == false) {
    log_info("Insert url[%s] to file map error", url.c_str());
    delete e;
    e = NULL;
  }
  updateMapCache();
...
int FileManager::downloadFileByUrl(char* url, void* buf, u32* size) {
  ...
  if (!en) {
    log_info("download file error, while url:%s", url);
    return -1;
  }
...
void FileManager::releaseFileCache(const std::string& url) {
  ...
  if (!entry) {
    log_info("map has not the url::%s", url.c_str());
    m_cs.Leave();
    return;
  }
