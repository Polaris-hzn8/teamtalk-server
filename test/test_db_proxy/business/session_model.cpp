/*
 ...
 filename: SessionModel.cpp
 Update Time: Thu 15 Jun 2023 01:07:57 CST
 brief:
*/
...
void CSessionModel::getRecentSession(
    uint32_t nUserId, uint32_t lastTime,
    list<IM::BaseDefine::ContactSessionInfo>& lsContact) {
  CDBManager* pDBManager = CDBManager::getInstance();
  CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
  if (pDBConn) {
    string strSql =
        "select * from IMRecentSession where userId = " + int2string(nUserId) +
        " and status = 0 and updated >" + int2string(lastTime) +
    ...
    if (pResultSet) {
      while (pResultSet->Next()) {
        ...
        cRelate.set_session_status(
            ::IM::BaseDefine::SessionStatusType(pResultSet->GetInt("status")));

        IM::BaseDefine::SessionType nSessionType =
            IM::BaseDefine::SessionType(pResultSet->GetInt("type"));
        if (IM::BaseDefine::SessionType_IsValid(nSessionType)) {
          ...
          log("invalid sessionType. userId=%u, peerId=%u, sessionType=%u",
...
uint32_t CSessionModel::getSessionId(uint32_t nUserId, uint32_t nPeerId,
                                     uint32_t nType, bool isAll) {
  CDBManager* pDBManager = CDBManager::getInstance();
  CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
  ...
  if (pDBConn) {
    ...
    if (isAll) {
      strSql =
          "select id from IMRecentSession where userId=" + int2string(nUserId) +
...
bool CSessionModel::updateSession(uint32_t nSessionId, uint32_t nUpdateTime) {
  ...
  if (pDBConn) {
    string strSql =
        "update IMRecentSession set `updated`=" + int2string(nUpdateTime) +
        " where id=" + int2string(nSessionId);
...
bool CSessionModel::removeSession(uint32_t nSessionId) {
  ...
  if (pDBConn) {
    ...
    string strSql =
        "update IMRecentSession set status = 1, updated=" + int2string(nNow) +
        " where id=" + int2string(nSessionId);
    bRet = pDBConn->ExecuteUpdate(strSql.c_str());
    pDBManager->RelDBConn(pDBConn);
  } else {
...
uint32_t CSessionModel::addSession(uint32_t nUserId, uint32_t nPeerId,
                                   uint32_t nType) {
  uint32_t nSessionId = INVALID_VALUE;
  ...
  if (pDBConn) {
    if (INVALID_VALUE != nSessionId) {
      ...
      string strSql =
          "insert into IMRecentSession "
          "(`userId`,`peerId`,`type`,`status`,`created`,`updated`) "
          "values(?,?,?,?,?,?)";
      // 必须在释放连接前delete
...
void CSessionModel::fillSessionMsg(
    uint32_t nUserId, list<IM::BaseDefine::ContactSessionInfo>& lsContact) {
  for (auto it = lsContact.begin(); it != lsContact.end();) {
    ...
    if (it->session_type() == IM::BaseDefine::SESSION_TYPE_SINGLE) {
      ...
      CMessageModel::getInstance()->getLastMsg(it->session_id(), nUserId,
                                               nMsgId, strMsgData, nMsgType);
    } else {
      CGroupMessageModel::getInstance()->getLastMsg(
          it->session_id(), nMsgId, strMsgData, nMsgType, nFromId);
    }
